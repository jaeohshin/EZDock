...
license_path = '/data/work/EZDock/OpenEye/oe_license.txt'
...
class ReceptorSelect(Select):
    ...
    def accept_residue(self, residue):
        ...
        for atom in residue:
            '''in biopython, atom.get_full_id()[3] is accessing the fourth element of the full ID. This element represents the atom name, 
            atom.get_full_id()[3][0] is accessing the first character of the atom name. This character represents the element symbol of the atom. 
            The condition atom.get_full_id()[3][0] == ' ' checks whether the first character of the atom name is a space. If it is a space, then the atom is from an ATOM record, 
            otherwise it is from a HETATM record.'''
...
def vina_process_lig_prot(lig_file, prot_file, complex_file, preserve_water=False, preserve_metal=False, csv_out_file='vina_docking_data.csv'):
    ...
    try:      
        os.environ['OE_LICENSE'] = '/data/work/EZDock/OpenEye/oe_license.txt' # change this to your OE_LICENSE path
...
################################ VINA PIPELINE ################################
import glob
all_ligands = glob.glob('./examples/BALM/Updated_Mpro_ligands/*.sdf') # change this to the path where the ligands are stored
...
for ligand_file in all_ligands:
    ...
    os.system(f'cp ./examples/BALM/Mpro_complexes/{receptor_file} .')
    os.system(f'cp ./examples/BALM/Mpro_complexes/{complex_file} .')
    ...
    vina_process_lig_prot(ligand_file, 
                        ...
                        preserve_metal=True, 
                        csv_out_file='mpro_vina_w_water_data_v2_2.csv')