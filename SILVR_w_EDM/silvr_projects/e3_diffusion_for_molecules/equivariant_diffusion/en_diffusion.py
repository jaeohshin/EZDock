...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    assert_correctly_masked, sample_center_gravity_zero_gaussian_with_mask
        ...
        for s in reversed(range(0, self.T)):
            ...
            #BRIDGE equations
            # refinement 
            reference_bias = silvr_rate

            z = (1-reference_bias)*z + reference_bias*ref_node_mask


            # # TODO: refinement_sequential?
            # s_int = int(s_array.item())
            # maxT= max(self.T)
            # # turn above into lambda function
            # drop_bias = lambda bias, s_int, maxT: bias*(s_int)/maxT
            # reference_bias = drop_bias(reference_bias, s_int, maxT)
